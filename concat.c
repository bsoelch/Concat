...
bool isPointerType(const DataType* type){
  return type->typeClass==TYPECLASS_MUTABLE_POINTER||type->typeClass==TYPECLASS_CONST_POINTER;
}
  ...
  for(int32_t i=0;i<compositeCount;i++){
    if(compositeTypes[i].typeCount==eltCount||(typeMatch==-1&&compositeTypes[i].typeCount>eltCount)){
      ...
      if(compositeTypes[i].typeCount==eltCount){
        if(labelOffset!=-1&&compositeTypes[i].labelOffset==-1){
    ...
    case TYPECLASS_ENUM_LABEL:
      ...
      for(int32_t e=0;e<type->typeDataAs.composite->typeCount;e++){
        if((type->typeClass==TYPECLASS_ENUM||type->typeClass==TYPECLASS_ENUM_LABEL)&&isVoidType(&(type->typeDataAs.composite->types[e]))){
          ...
          j=fprintf(file," %.*s",(int)labelName.length,labelName.chars);
          if(j<0)
            return j;
          i+=j;
          continue;
...
DataType progStringType(void){
    ...
    DataType stringElts[2]={pointerType(&chr,false),primitiveType(PRIMITIVE_I64)};
    return compositeType(TYPECLASS_STRUCT,stringElts,stringLabelOffset,2);//ensure string-type exists
}
...
bool readType(String name,CodeFile* codeFile,CompilerState* state){
  ...
  if(wordEquals(&name,"mut")){//XXX mutable tuples/structs/enums
    ...
    if(!isPointerType(&typeBuffer[bufferedTypes-1]))
      handleError("mut can only be used on pointer types",ERROR_TYPE,codeFile->wordStart);
    if(typeBuffer[bufferedTypes-1].typeClass==TYPECLASS_MUTABLE_POINTER)
      handleError("type is already mutable",ERROR_TYPE,codeFile->wordStart);
    typeBuffer[bufferedTypes-1].typeClass=TYPECLASS_MUTABLE_POINTER;
    return true;
  }
...
void addCompiledStackOps(TypeCheckState* state,Operation op,size_t stackOps,size_t types,bool appendOp){
...
void addCompiledOp(TypeCheckState* state,Operation op,size_t types){
  addCompiledStackOps(state,op,types,types,true);
}

void compileCompositeOp(TypeCheckState* state,DataType* type,Operation* ops,size_t nOps,int32_t tmpId){
  if(nOps==0)
    return;
...
void extractCompositeOpsOffset(TypeCheckState* state,size_t nStackValues,size_t skipValues){
  ...
  for(size_t i=state->typeCount-nStackValues-skipValues;i<state->typeCount-skipValues;i++){
    ...
    if(state->typeStack[i].opCount>1||(state->opStack[offset].opType==OP_CONSTANT&&isArrayType(&(state->opStack[offset].dataType)))){
      ...
      compileCompositeOp(state,&(state->typeStack[i].type),state->opStack+offset,state->typeStack[i].opCount,tmpId);
...
void extractCompositeOps(TypeCheckState* state,size_t nStackValues){
  extractCompositeOpsOffset(state,nStackValues,0);
}
...
bool canAssign(const DataType* src,const DataType* target){
  ...
  if(isPointerType(src)&&target->typeClass==TYPECLASS_CONST_POINTER&&typeEquals(src->typeDataAs.type,target->typeDataAs.type))
...
void requireTypes(const char* opName,TypeCheckState* state,DataType* types,size_t nTypes,FilePosition pos){
  ...
  if(nCasts==0)
    return;
  //extract composites
  extractCompositeOps(state,nTypes);
  //modify-types
...
void typeCheckCall(Operation* op,TypeCheckState* state,bool isPtr){
  ...
  //extract operations
  extractCompositeOps(state,argCount+(isPtr?1:0));
  int32_t tmpId;
  ...
  addCompiledOp(state,*op,isPtr?1:0);
  ...
  //update op-stack
  addCompiledStackOps(state,*op/*ignored*/,argCount,argCount,false);
    ...
    case ID_POINTER_OFFSET:
      ...
      if(isArrayType(&(state->typeStack[offset].type))){//handle wrapped array types
        ...
        pushCompiledOperation(state,(Operation){.opType=OP_GET,.dataType=arrayTypeData->types[1],.filePos=op->filePos,
          .dataAs={.idInfo={.type=ID_TUPLE_ELEMENT,.id=1,.labelId=arrayTypeData->labelOffset+1,.isMutable=false}}});
        ...
        pushCompiledOperation(state,opGetIntermediate(&(arrayTypeData->types[1]),lenIndex,op->filePos));//length
        ...
        pushCompiledOperation(state,opGetIntermediate(&(arrayTypeData->types[0]),ptrIndex,op->filePos));//pointer
  ...
  switch(op.opType){
      ...
      switch(op.dataAs.idInfo.type){
        ...
        case ID_GLOBAL_VAR:
          ...
          if(state->blockCount==0){//don't extract operations at global level
            addCompiledStackOps(state,op,state->typeStack[offset].opCount,1,true);
            return;
          }
          extractCompositeOps(state,1);
          addCompiledOp(state,op,1);
    ...
    case OP_NEW:
      ...
      if(op.dataType.typeClass==TYPECLASS_TUPLE||op.dataType.typeClass==TYPECLASS_STRUCT){
        ...
        //store result in temp variable
        extractCompositeOps(state,op.dataType.typeDataAs.composite->typeCount);
        tmpId=state->tmpCount++;
        pushCompiledOperation(state,opDeclareIntermediate(&op.dataType,tmpId,op.filePos));
        addCompiledOp(state,op,op.dataType.typeDataAs.composite->typeCount);
        //update stack
        pushValue(state,opGetIntermediate(&op.dataType,tmpId,op.filePos));
        return;
      }
